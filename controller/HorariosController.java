...
public class HorariosController {
	...
	public String guardarHorario(@RequestBody SesionDTO horarioDTO, HttpSession session) throws ParseException {
		...
		if (horarioDTO.getIdHorario() > 0) {
			...
			Horario horarioProfesor = horarioService.buscarPorHoraInicioHorafinYPeriodoYProfesorYDia(horaInicioDTO,
					horaFinDTO, usuario.getPreferencias().getIdPeriodo(), cargaHoraria.getProfesor().getId(),
					horarioDTO.getDia());
			if (horarioProfesor != null) {
				return "hp";
			}
			Horario horarioGrupo = horarioService.buscarPorHoraInicioHoraFinYGrupoYdia(horaInicioDTO, horaFinDTO,
					cargaHoraria.getGrupo().getId(), horarioDTO.getDia());
