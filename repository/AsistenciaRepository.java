...
public interface AsistenciaRepository extends CrudRepository<Asistencia, Integer> {
	...
	// cuenta el numero de asistencias por alumno, materia y corte(fecha de inicio y
	// fecha de fin)
	@Query(value = "SELECT COUNT(DISTINCT(a.id)) as asistencias FROM asistencias a "
			+ "INNER JOIN horarios h ON h.id = a.id_horario " + "WHERE h.id_carga_horaria = :idCargaHoraria "
			+ "AND id_alumno = :idAlumno " + "AND a.fecha BETWEEN :fechaInicio and :fechaFin "
			+ "AND (a.valor = 'A' OR a.valor = 'J' OR a.valor = 'R') ", nativeQuery = true)
	Integer countAsistenciasByAlumnoAndCargaHorariaAndCorteEvalutivo(@Param("idAlumno") Integer idAlumno,
			@Param("idCargaHoraria") Integer idCargaHoraria, @Param("fechaInicio") Date fechaInicio,
			@Param("fechaFin") Date fechaFin);

	// cuenta el numero de faltas por alumno, materia y corte (fecha inicio y fecha
	// fin)
	@Query(value = "SELECT COUNT(DISTINCT(a.id)) as asistencias FROM asistencias a "
			+ "INNER JOIN horarios h ON h.id = a.id_horario " + "WHERE h.id_carga_horaria = :idCargaHoraria "
			+ "AND a.fecha BETWEEN :fechaInicio and :fechaFin "
			+ "AND a.valor = 'F' AND id_alumno = :idAlumno", nativeQuery = true)
	Integer countFaltasByAlumnoAndCargaHorariaAndCorteEvalutivo(@Param("idAlumno") Integer idAlumno,
			@Param("idCargaHoraria") Integer idCargaHoraria, @Param("fechaInicio") Date fechaInicio,
			@Param("fechaFin") Date fechaFin);
