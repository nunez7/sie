...
public interface AlumnosRepository extends CrudRepository<Alumno, Integer> {
	...
	@Query(value = "SELECT COUNT(*) as inscritos " + "FROM alumnos_grupos ag "
	...
	@Query(value = "SELECT COUNT(ag.id) as inscritos " + "FROM alumnos_grupos ag "
			+ "INNER JOIN alumnos a on a.id=ag.id_alumno " + "INNER JOIN grupos g ON g.id=ag.id_grupo "
	...
	Integer countInscritosByCarreraAndPeriodo(@Param("idCarrera") Integer idCarrera,
	...
	@Query(value = "SELECT a.* " + "FROM alumnos_grupos ag " + "INNER JOIN alumnos a ON a.id=ag.id_alumno "
			+ "INNER JOIN personas p ON p.id=a.id_persona " + "WHERE ag.id_grupo= :idGrupo AND ag.activo = 'True' "
	...
	@Query(value = "SELECT COUNT(a.id)AS cantidad " + "FROM alumnos_grupos ag "
			+ "INNER JOIN alumnos a ON a.id=ag.id_alumno " + "INNER JOIN grupos g ON g.id=ag.id_grupo "
	...
	@Query(value = "SELECT a.id as idAlumno, CONCAT(p.nombre,' ',p.primer_apellido,' ',p.segundo_apellido) as alumno, a.matricula, pg.concepto, pg.monto as cantidad "
			...
			+ "ORDER BY p.nombre, p.primer_apellido, p.segundo_apellido", nativeQuery = true)
	List<AlumnoAdeudoDTO> getAllAlumnoAdeudoByPersonaCarreraAndPeriodo(@Param("persona") Integer idPersona,
	...
	@Query(value = "SELECT a.id as idAlumno, CONCAT(p.nombre,' ',p.primer_apellido,' ',p.segundo_apellido) as alumno, a.matricula, c.concepto, pg.monto as cantidad, pg.concepto as descripcion "
			...
			+ "INNER JOIN alumnos a on pa.id_alumno=a.id " + "INNER JOIN alumnos_grupos ag ON ag.id_alumno = a.id "
			+ "INNER JOIN grupos g ON g.id = ag.id_grupo " + "INNER JOIN personas p ON p.id = a.id_persona "
			+ "INNER JOIN conceptos c ON c.id = pg.id_concepto " + "WHERE a.id_carrera=:carrera "
			+ "AND pg.status=0 AND g.id_periodo = :periodo "
			+ "ORDER BY p.nombre, p.primer_apellido, p.segundo_apellido", nativeQuery = true)
	List<AlumnoAdeudoDTO> getAllAlumnoAdeudoByCarreraAndPeriodo(@Param("carrera") Integer idCarrera,
			@Param("periodo") Integer idPeriodo);
	...
	@Query(value = "SELECT a.id AS idAlumno, a.matricula, p.primer_apellido AS primerApellido, p.segundo_apellido AS segundoApellido,  "
			+ "p.nombre,  a.id_persona AS idPersona, g.nombre AS grupoAnterior, " + "(SELECT COALESCE(gg.nombre, '') "
			+ "FROM alumnos_grupos agg " + "INNER JOIN grupos gg ON gg.id=agg.id_grupo "
			+ "WHERE gg.id_periodo=:periodo+1 AND gg.id_carrera=:carrera AND agg.id_alumno=a.id)AS grupoActual "
			+ "FROM alumnos a " + "INNER JOIN alumnos_grupos ag ON ag.id_alumno=a.id "
			+ "INNER JOIN grupos g ON g.id=ag.id_grupo " + "INNER JOIN cuatrimestres cu ON cu.id=g.id_cuatrimestre "
			+ "INNER JOIN carreras c ON c.id=g.id_carrera " + "INNER JOIN personas p ON p.id=a.id_persona "
			...
			+ "AND ag.pagado = 'True' AND ag.fecha_inscripcion IS NOT NULL " + " AND a.id NOT IN ("
			+ " SELECT id_alumno FROM pago_alumno pa " + " INNER JOIN pagos_generales pg  ON pg.id=pa.id_pago "
	...
	List<AlumnoRegularDTO> findAllRegular(@Param("carrera") Integer carrera, @Param("periodo") Integer periodo,
	...
	@Query(value = "SELECT a.id AS idAlumno, a.matricula, p.primer_apellido AS primerApellido, p.segundo_apellido AS segundoApellido,  "
			...
			+ " WHERE pa.id_alumno=a.id AND pg.status=0) "
			+ "ORDER BY c.nombre, g.nombre DESC, p.primer_apellido, p.segundo_apellido, p.nombre", nativeQuery = true)
	List<AlumnoRegularDTO> obtenerRegularesByCarreraPeriodo(@Param("carrera") Integer carrera,
	...
	@Query(value = "SELECT a.id AS idAlumno, a.matricula, CONCAT(p.primer_apellido, ' ',p.segundo_apellido, ' ',p.nombre)AS nombreCompleto,  c.nombre AS carrera, "
			...
			+ "SELECT MAX(status) FROM pagos_generales pg " + "INNER JOIN pago_alumno pa ON pa.id_pago =pg.id "
			...
			+ "CAST(da.dialecto AS INT) , da.promocion , da.tipo_beca as tipoBeca " + "FROM alumnos a "
			+ "INNER JOIN datos_alumno da ON da.id_alumno=a.id " + "INNER JOIN personas p ON p.id=a.id_persona "
			+ "INNER JOIN carreras c ON c.id=a.id_carrera " + "LEFT JOIN datos_personales dp ON dp.id_persona=p.id "
			...
			+ "LEFT JOIN estados eb ON eb.id=esc.id_estado " + "LEFT JOIN estados e ON e.id=dp.edo_nacimiento "
	...
	@Query(value = "SELECT a.id AS idAlumno,  CONCAT(p.primer_apellido, ' ',p.segundo_apellido, ' ',p.nombre)AS nombreCompleto, a.matricula, "
			...
			+ "COALESCE(( " + "	SELECT cmm.estatus FROM calificacion_materia  cmm "
			...
			+ "	ORDER BY cmm.estatus ASC " + "	LIMIT 1 " + "), 'O')AS estatus " + "FROM alumnos a "
			+ "INNER JOIN personas p ON p.id=a.id_persona " + "INNER JOIN alumnos_grupos ag ON ag.id_alumno=a.id "
			+ "INNER JOIN grupos g ON g.id=ag.id_grupo " + "INNER JOIN cuatrimestres c ON c.id=g.id_cuatrimestre "
			+ "INNER JOIN carreras ca ON ca.id=g.id_carrera " + "LEFT JOIN cargas_horarias ch ON ch.id=g.id "
	...
	@Query(value = "SELECT a.id AS idAlumno, CONCAT(p.primer_apellido, ' ',p.segundo_apellido, ' ',p.nombre)AS nombreCompleto, a.matricula, "
			...
			+ "GROUP BY g.id, ca.nombre, c.descripcion, g.nombre, p.primer_apellido, p.segundo_apellido, p.nombre, a.id "
			+ "ORDER BY g.nombre, p.primer_apellido, p.segundo_apellido, p.nombre", nativeQuery = true)
	List<AlumnoPromedioEscolaresDTO> findAllPromedioEscolaresByPeriodoAndCarrera(@Param("periodo") Integer periodo,
	...
	@Query(value = "SELECT a.id AS idAlumno, a.matricula, CONCAT(p.primer_apellido, ' ',p.segundo_apellido, ' ',p.nombre)AS nombreCompleto, "
			...
			+ "FROM alumnos a " + "INNER JOIN personas p ON p.id=a.id_persona "
			+ "INNER JOIN alumnos_grupos ag ON ag.id_alumno=a.id " + "INNER JOIN grupos g ON g.id=ag.id_grupo "
			+ "INNER JOIN cuatrimestres c ON c.id=g.id_cuatrimestre " + "INNER JOIN carreras ca ON ca.id=g.id_carrera "
			+ "LEFT JOIN datos_alumno da ON da.id_alumno=a.id " + "LEFT JOIN datos_personales dp ON dp.id_persona=p.id "
			+ "LEFT JOIN estados e ON e.id=dp.edo_nacimiento "
			+ "LEFT JOIN escuela_procedencia ep ON ep.id_alumno=a.id "
			+ "LEFT JOIN escuelas esc ON esc.id=ep.id_escuela_procedencia "
			+ "LEFT JOIN estados eb ON eb.id=esc.id_estado " + "WHERE g.id_periodo=:periodo AND ca.id=:carrera "
			+ "ORDER BY ca.nombre, ca.id_nivel_estudio, g.nombre, p.primer_apellido, p.segundo_apellido, p.nombre", nativeQuery = true)
	List<AlumnoMatriculaInicialDTO> findAllMatriculaInicial(@Param("periodo") Integer periodo,
			@Param("carrera") Integer carrera);

	// GADIEL
	@Query(value = "SELECT LPAD(CAST(SUM(total) AS VARCHAR),4,'0')"
			+ " FROM (SELECT COUNT(a.matricula)+1 AS total FROM  alumnos a" + "	WHERE a.matricula ILIKE %:matri%"
			+ " UNION ALL" + " SELECT COUNT(rc.clave) AS total FROM reserva_claves rc"
			+ "	WHERE rc.clave ILIKE %:matri%" + "	)AS total", nativeQuery = true)
	...
	@Query(value = "SELECT (a.*) FROM alumnos a" + " LEFT JOIN alumnos_grupos ag ON ag.id_alumno = a.id"
			+ " LEFT JOIN pago_alumno pa ON pa.id_alumno = a.id" + " LEFT JOIN pagos_generales pg ON pg.id = pa.id_pago"
			+ " LEFT JOIN persona_documento pd ON pd.id_persona = a.id_persona"
			+ " WHERE ag.id_alumno IS NULL AND pa.id_pago = pg.id AND pg.status = 1" + " GROUP BY a.id"
			+ " HAVING COUNT(pd.id_persona)>=3", nativeQuery = true)
	List<Alumno> findAllByStatusPagoGeneralWithoutGrupoAndWithDocumentos();

	@Query(value = "SELECT a.* FROM alumnos a" + " LEFT JOIN alumnos_grupos algr on algr.id_alumno = a.id"
			+ " LEFT JOIN grupos gr on gr.id = algr.id_grupo"
			+ " WHERE gr.id_periodo=:periodo and a.id_carrera =:carrera" + " ORDER BY gr.id ASC", nativeQuery = true)
	...
	List<Alumno> findAllAceptarByCarreraAndPeriodo(@Param("carrera") Integer idCarrea,
			@Param("periodo") Integer idPeriodo);

	@Modifying(clearAutomatically = true)
	@Transactional
	@Query(value = "INSERT INTO reserva_claves (clave)" + " VALUES" + " (:matri)", nativeQuery = true)
	void insertMatriculaFromAlumnoInReservaClave(@Param("matri") String matri);

	@Query(value = "select a.* " + "FROM alumnos a " + "INNER JOIN personas per ON per.id=a.id_persona "
			+ "WHERE CONCAT(per.nombre,' ',per.primer_apellido) iLIKE %:nombre% "
			+ "OR CONCAT(per.primer_apellido,' ',per.segundo_apellido) iLIKE %:nombre% "
			+ "OR CONCAT(per.segundo_apellido, ' ',per.nombre) iLIKE %:nombre% " + "OR a.matricula iLIKE %:nombre% "
	...
	@Query(value = "select a.*,CONCAT(per.nombre,' ',per.primer_apellido,' ',per.segundo_apellido) AS nombreCompleto "
			+ "FROM alumnos a " + "INNER JOIN alumnos_grupos ag ON a.id=ag.id_alumno "
			...
			+ "OR CONCAT(per.segundo_apellido, ' ',per.nombre) iLIKE %:nombre% " + "OR a.matricula iLIKE %:nombre% "
			+ "ORDER BY per.primer_apellido, per.segundo_apellido, per.nombre", nativeQuery = true)
	List<AlumnoInfoDTO> findByProfesorAndPeriodoAndNombreOrMatricula(@Param("idProfesor") Integer idProfesor,
			@Param("idPeriodo") Integer idPeriodo, @Param("nombre") String nombre);

	@Query(value = "select a.*,CONCAT(per.nombre,' ',per.primer_apellido,' ',per.segundo_apellido) AS nombreCompleto "
			+ "FROM alumnos a " + "INNER JOIN alumnos_grupos ag ON a.id=ag.id_alumno "
			+ "INNER JOIN grupos g ON g.id=ag.id_grupo " + "INNER JOIN personas per ON per.id=a.id_persona "
			+ "WHERE AND g.id_profesor=:idProfesor AND g.id_periodo=:idPeriodo "
			+ "ORDER BY per.primer_apellido, per.segundo_apellido, per.nombre", nativeQuery = true)
	List<AlumnoInfoDTO> findByProfesorAndPeriodo(@Param("idProfesor") Integer idProfesor,
			@Param("idPeriodo") Integer idPeriodo);

	@Query(value = "SELECT a.* FROM alumnos_grupos ag " + "INNER JOIN grupos g ON g.id=ag.id_grupo "
			+ "INNER JOIN alumnos a ON a.id=ag.id_alumno " + "INNER JOIN personas p ON p.id=a.id_persona "
	...
	@Query(value = "SELECT a.* FROM alumnos a " + "INNER JOIN personas p ON a.id_persona = p.id "
			...
			+ "ORDER BY p.nombre, p.primer_apellido, p.segundo_apellido", nativeQuery = true)
	List<Alumno> findAllAlumnosByPersonaCarreraAndActivoAndPeriodo(@Param("idPersona") Integer idPersona,
			@Param("idPeriodo") Integer idPeriodo);

	@Query(value = "SELECT a.* FROM alumnos a " + "INNER JOIN personas p ON a.id_persona = p.id "
			+ "INNER JOIN alumnos_grupos ag ON ag.id_alumno = a.id " + "INNER JOIN grupos g ON g.id = ag.id_grupo "
			+ "WHERE g.id_carrera = :idCarrera AND estatus = 1 " + "AND g.id_periodo = :idPeriodo "
			+ "GROUP BY a.id, p.nombre, p.primer_apellido, p.segundo_apellido "
			+ "ORDER BY p.nombre, p.primer_apellido, p.segundo_apellido", nativeQuery = true)
	List<Alumno> findAllAlumnosByCarreraAndActivoAndPeriodo(@Param("idCarrera") Integer idCarrera,
			@Param("idPeriodo") Integer idPeriodo);

	@Query(value = "SELECT count(a.id) " + "FROM alumnos a " + "INNER JOIN alumnos_grupos ag ON ag.id_alumno=a.id "
			+ "INNER JOIN grupos g ON g.id=ag.id_grupo " + "INNER JOIN cuatrimestres cu ON cu.id=g.id_cuatrimestre "
			+ "INNER JOIN carreras c ON c.id=g.id_carrera " + "INNER JOIN personas p ON p.id=a.id_persona "
	...
	@Query(value = "SELECT a.id AS idAlumno, a.matricula, CONCAT(p.primer_apellido, ' ',p.segundo_apellido, ' ',p.nombre)AS nombreCompleto,  c.nombre AS carrera, c.id as idCarrera, "
			...
			+ "SELECT MAX(status) FROM pagos_generales pg " + "INNER JOIN pago_alumno pa ON pa.id_pago =pg.id "
			+ "WHERE pa.id_alumno=a.id AND pg.id_concepto=12 " + "), 0) AS INTEGER ) AS pago, p.id as idPersona "
			+ "FROM alumnos a " + "INNER JOIN personas p ON p.id=a.id_persona "
			+ "INNER JOIN carreras c ON c.id=a.id_carrera " + "LEFT JOIN datos_personales dp ON dp.id_persona=p.id "
			+ "LEFT JOIN alumnos_grupos ag ON ag.id_alumno=a.id " + "WHERE ag.id_alumno IS NULL AND a.estatus = 1 "
	...
	@Query(value = "SELECT a.* " + "FROM alumnos a " + "INNER JOIN personas p ON p.id=a.id_persona "
			+ "INNER JOIN carreras c ON c.id=a.id_carrera " + "INNER JOIN alumnos_grupos ag ON ag.id_alumno=a.id "
	...
	@Query(value = "SELECT DISTINCT(a.id) as idalumno, CONCAT(p.nombre,' ',p.primer_apellido,' ',p.segundo_apellido) as nombre, "
			...
			+ "FROM alumnos_grupos ag " + "INNER JOIN alumnos a on a.id = ag.id_alumno "
			+ "INNER JOIN grupos g ON g.id = ag.id_grupo " + "INNER JOIN carreras c ON c.id = g.id_carrera "
	...
	List<AlumnoNoReinscritoDTO> findNoReinscritosByPersonaCarreraAndPeriodo(@Param("idPersona") Integer idPersona,
			@Param("idPeriodo") Integer idPeriodo);
